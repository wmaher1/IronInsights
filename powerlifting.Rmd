...
```{r lasso}
y <- usapl_data$TotalKg
x <- data.matrix(usapl_data[, c('Squat1Kg', 'Bench1Kg', 'Deadlift1Kg')])
...
```{r squat}
squat_lm <- lm(Best3SquatKg ~ Squat1Kg + Squat2Kg + Squat3Kg, usapl_data2)
summary(squat_lm)
```

```{r bench}
bench_lm <- lm(Best3BenchKg ~ Bench1Kg + Bench2Kg + Bench3Kg, usapl_data2)
summary(bench_lm)
```

```{r deadlift}
deadlift_lm <- lm(Best3DeadliftKg ~ Deadlift1Kg + Deadlift2Kg + Deadlift3Kg,
                  usapl_data2)
summary(deadlift_lm)
```
